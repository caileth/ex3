function aim(id, target) {
  ...
  RESULTS_WINDOW.append(attacker.name + " takes Aim against " +
                        defender.name + "&hellip;\n");
...
function attack(id, target) {
  ...
  var attackIsDecisive =
          $("input[name=attackIsDecisive]:checked").val() === "true",
  ...
  var defendSpecialty = $("#defendSpecialty").prop('checked'),
  ...
  // apply wound penalties if applicable
  if (!isNaN(attackWound)) attackModifiers += attackWound;

  // apply aim bonus if applicable
  if (attacker.aimTarget === defender) attackModifiers += AIM_BONUS;

  // reset aim target for attacker either way
  attacker.aimTarget = undefined;

  if (attacker.initiative != attackTick)
    attacker.initiative += DELAYED_ATTACK_PENALTY;

  var newAttack = new PendingAttack(attackTick, attacker, defender,
                                    attackModifiers, attackStunt, defendStunt,
                                    attackIsDecisive, attackSpecialty,
                                    defendSpecialty);

  SCENE.pendingAttacks.push(newAttack);
  ...
  console.log(defender.name + " is now at " + defender.onslaught +
              " Onslaught penalty");
...
function resolveAttack(attack) {
  console.groupCollapsed("Resolving " + attack.attacker.name +
                          "'s attack versus " + attack.defender.name);

  
  // presumably will add Charm hooks here eventually
  var attackAuto = attack.attackStunt.auto,
      attackPool = attack.attackModifiers + attack.attackStunt.dice,
      targetDefense = attack.defender.getDefense(attack.defendSpecialty) +
          attack.defendStunt.static;
  ...
  if (attack.attackStunt.level > 0) {
    RESULTS_WINDOW.append(attack.attacker.name + " uses a " +
        attack.attackStunt.level + "-point stunt!\n");
  }

  if (attack.defendStunt.level > 0) {
    RESULTS_WINDOW.append(attack.defender.name + " uses a " +
        attack.defendStunt.level + "-point stunt!\n");
  }

  if (attack.isDecisive) {
    attackPool += attack.attacker.getDecisivePool();
  } else {
    attackPool += attack.attacker.getWitheringPool();
  }

  if (attack.attackSpecialty) attackPool += SPECIALTY_DIE_BONUS;

  var summary = attack.attacker.name + " attempts a " +
                (attack.isDecisive ? "Decisive" : "Withering") +
                " Attack (" + attackPool + " dice) against " +
                attack.defender.name + " (" + targetDefense + " defense)!\n";

  RESULTS_WINDOW.append(summary);

  makeAttackRoll(attack.attacker, attack.defender, attackAuto,
                  attackPool, targetDefense, attack.isDecisive);
